<?php
  ...
  $message = '';
  if (!empty($_POST['nombre']) && !empty($_POST['correo']) && !empty($_POST['contrasenha']) && !empty($_POST['interes']) && $_POST['contrasenha'] = $_POST['cc']) {
    $sql = "INSERT INTO usuarios (nombre, contrasenha, correo, interes) VALUES (:nombre, :contrasenha, :correo, :interes)";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':nombre', $_POST['nombre']);
    $password = password_hash($_POST['contrasenha'], PASSWORD_BCRYPT);
    $stmt->bindParam(':contrasenha', $password);
    $stmt->bindParam(':correo', $_POST['correo']);
    $stmt->bindParam(':interes', $_POST['interes']);
    if ($stmt->execute()) {
      $message = '¡Su usuario se ha creado exitosamente!';
    } else {
      $message = 'Lo sentimos, pero no pudimos crear su usuario.';
    }
  }
?>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-offset-2 col-md-8 col-sm-12">
				...
				<a href="#contact" class="btn btn-warning section-btn smoothScroll hidden-xs wow fadeInUp" data-wow-delay="1.9s">DESCARGAR VISUALBOY</a>
                                <?php if(empty($user)): ?>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-6 col-sm-10 wow fadeInUp" data-wow-delay="1.3s">
				<div class="section-title">
				    <h1><BR><BR>Reseñas</h1>
					<p>Aquí estan todas las reseñas de la página. Puede usar el buscador para encontrar la que guste</p><BR>
				</div>
				<input type="text" class="form-control" placeholder="Título" name="titulo" required><BR>
			    <A HREF="reseña_beta1.HTML">Super Mario Advance</A><BR>
			    <A HREF="reseña_beta2.HTML">TLOZ: Minish cap</A><BR>
			    <A HREF="reseña_beta3.HTML">Pokémon: Oro y plata</A>
			</div>
			<div class="col-md-16 col-sm-16 wow fadeInUp" data-wow-delay="1.6s">
			...
			<div class="col-md-offset-2 col-md-8 col-sm-offset-1 col-sm-10 wow fadeIn" data-wow-delay="0.6s">
				<form action="registrado.php" method="POST">
					<div class="col-md-6 col-sm-6">
						...
						<input name="contrasenha" type="password" value="FakePSW" class="form-control" placeholder="Contraseña" required>
						<input name="cc" type="password" value="FakePSW" class="form-control" placeholder="Confirmar contraseña" required>
					</div>
					<div class="col-md-6 col-sm-6">
